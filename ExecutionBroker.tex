...
\subsubsection{Metadata roles}
\label{metadata-roles}

The full description of an \executablething{} may include several layers of metadata
...
The user, or the user's client agent, starts with the initial \metadoc{} from the
...
The \execbrokerservice{} takes this into account when calculating the start time of
...
The user can also update the compute resource requirements set by the software developer
to provide additional details based on how they plan to use the software.
...
communicate to the client what operations the user is able to do
...
\begin{itemize}
    ...
    \item \codeword{PROPOSED}   An internal state .. not published
