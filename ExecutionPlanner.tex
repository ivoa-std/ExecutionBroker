...
to outline the structure and comments to describe the elements.
...
The main part of the \execplanner{} \datamodel{} is the \codeword{request},
which describes an \executablething{} and the resources needed to execute it.

The request element is used in several places in the \datamodel{}:
\begin{itemize}[]
    \item It is main body of the initial \execplanner{} request call.
    \item The the \codeword{request} is repeated in each \codeword{offer} in an \execplanner{} response.
    \item The the \codeword{request} also occurs in an \execworker{} \job{} record.
\end{itemize}

\subsubsection{Executable}
\label{datamodel-executable}

The \codeword{executable} section of the \codeword{request} describes the \executablething{}.

The core \datamodel{} defines this as a URI identifying the \codeword{type} of executable,
...
Out of the box, the core \datamodel{} defines 4 types of \executable{};
\jupyter{} and \zeppelin{} notebooks, and OCI (Docker) and \singularity{} containers.
However, the structure is designed to make it easy for 3rd parties to define their own
types of \executable{}.
...
the endpoint address of the repository, \codeword{repo}, to dowload the
container image from, the \codeword{name} of the image and the \codeword{tag}
or \codeword{digest} identifier for the image.

The \datamodel{} extension for an \ocicontainer{} also includes
optional an \codeword{platform} element which describes the operating system
...
request:
  executable:
    ...
    spec:
      ...
      image: <string>
      # The image tag to specify the version to download.
      tag: <string>
      # The image digest to specify the version to download.
      digest: <string>
      # Details of the platforms that the container is built for.
      platforms:
      - # The host system architecture the container is built for (default "amd64").
        architecture: <string>
        # The host operating system the container is built for (default "linux").
        os: <string>
\end{lstlisting}
...
\begin{lstlisting}[]
https://www.purl.org/ivoa.net/executable-types/singulatiry-container
\end{lstlisting}
...
The second section of the request \codeword{body} describes the resources needed
to execute the task.
...
This structure enables many-to-many relationships between the \codeword{compute}
...
The \codeword{compute} \codeword{resources} section lists
the computing resources needed.

The core \datamodel{} part of a \codeword{compute} resource contains
a name for the resource that is unique within this request,
...
request:
  resources:
    ...
    - # A unique name for this compute resource.
      name: <string>
      # A count of how many replicas are needed (defaults to 1).
...
The core \datamodel{} defines the URL to identify a generic computing resource:
\begin{lstlisting}[]
https://www.purl.org/ivoa.net/resource-types/generic-compute
\end{lstlisting}
    ...
    - # A unique name for this compute resource.
      ...
      spec:
        ...
        - # A unique name for this volume.
          name: <string>
          # A reference to the storage resource this volume is linked to.
          resource: <string>
...
A positive \execplanner{} service response contains \codeword{result}
...
A negative \execplanner{} service response contains \codeword{result}
set to \codeword{NO}, followed by an optional list of messages
explaining why the request was rejected.

Each \codeword{message} consists of a path to an element in the original
request and a text message explaining why the element was rejected.
